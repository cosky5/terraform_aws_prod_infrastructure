...
provider "aws" {
  version = "~> 3.0"
  region  = "us-east-1"
  access_key = "AKIAIJGGRDP2JSJTKHMQ"
  secret_key = "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"
...
resource "aws_network_interface" "web-server-mac" {
  subnet_id       = aws_subnet.subnet_01.id
...
resource "aws_eip" "two" {
  vpc                       = true
  network_interface         = aws_network_interface.web-server-mac.id
...
resource "aws_instance" "web-server-instance" {
  ...
  network_interface{
      device_index = 0
      network_interface_id = aws_network_interface.web-server-mac.id
  }
